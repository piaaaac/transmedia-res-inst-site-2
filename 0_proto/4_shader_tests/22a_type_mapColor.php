...
void main() {
  ...
  gl_FragColor = texture2D(depthImage, uv) + texture2D(originalImage, uv + mouse*vec2(1.0, 1.0) * depth.g); // LARGE
...
let imgPath = "images/";
let imgMap1, imgMap2, imgTypography;
let blendedDepthMap;
let myShader;
let sliders;
let ctx;
...
function preload () {
  ...
  imgMap1 = loadImage(imgPath + item1);
}
