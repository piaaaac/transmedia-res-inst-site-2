...
`;
  ...
  @font-face {
    ...
    font-weight: 300;
    font-style: normal;
    src: url("fonts/GTCinetypeLight.otf") format("opentype"),
         url("fonts/GTCinetypeLight.woff") format("woff"),
         url("fonts/GTCinetypeLight.woff2") format("woff2");
  }
  @font-face {
...
window.addEventListener("click", function () {
  ...
  var int = setInterval(() => { 
    ...
    randomizeScreens(sinNorm, 1, 0.05);
  }, tStep);
  setTimeout(function () {
    clearInterval(int);
    randomizeScreens(0.1, 1, 0.05); 
...
function randomizeScreens (probVal1, val1, val2) {
  for( var j = 0; j < layers.length; j++ ) {
    var layer = layers[ j ];
    layer.style.opacity = val2;
    if (Math.random() < probVal1) {
      layer.style.opacity = val1;
    }
  }
}
