...
void main() {
  ...
  gl_FragColor = (texture2D(colorImage, uv) + texture2D(originalImage, uv + mouse*vec2(1.0, 1.0) * depth.g)); // LARGE
...
function setup () {
  ...
  imgTypography.fill(random(255), random(255), random(255));
  imgTypography.image(imgColor, 0,0,width, height)
  writeTexts();
...
function draw() {
  ...
  myShader.setUniform("depthImage", get(m, m, width-m*2, height-m*2));
...
function writeTexts () {
  ...
  if (r < 0) {
    ...
    imgTypography.canvas.style.letterSpacing = -fs * 0.05 + "px";
    // ctx.font = '250px Arial';
    ctx.font = fs + 'px Martian Mono';
    imgTypography.text("TRANSMEDIA", width/2, height*0.5 - fs*1.1);
    imgTypography.text("RESEARCH", width/2, height*0.5);
    imgTypography.text("INSTITUTE", width/2, height*0.5 + fs*1.1);
  }  
}
